...
- **Batch 1（数据底座）**：已在 Ubuntu VM 验证通过（`prisma validate` 通过、迁移建 14 表、seed 幂等）。
- **Batch 2（N5 导入）**：已在 Ubuntu VM 验证通过——718 行全部导入、0 阻断错误、重复导入幂等（不增记录）、21 条验收样例全部匹配；读音表单 730（含 12 多读音）、表记表单 732（含 14 多表记）、`VocabularyMastery` 计数 0（未预创建）。
- **Batch 3（只读单词本）**：已在 Ubuntu VM 验证通过——typecheck、lint、`next build` 全绿；数据层冒烟确认按汉字（学校）、假名（がっこう）、替代表记（河→川）搜索均命中，分页与详情多读音正确。页面/HTTP 的浏览器人工验收（需登录）留待手动。
- **Batch 4（学习计划）**：已在 Ubuntu VM 随 Batch 5/6 回归验证——typecheck、lint、`next build` 全绿；独立数据库中可创建固定今日新词分配，Dashboard 可生成并统计到期复习任务。
- **Batch 5（客观题闭环）**：已在 Ubuntu VM 验证通过——会话创建、服务端出题、正确作答、重复提交返回 409、错误后隔三题重现全部通过；测试使用独立临时 SQLite 数据库。
- **Batch 6（复习闭环）**：已在 Ubuntu VM 验证通过——到期词进入 REVIEW 会话、正确作答推进复习调度、错词按时间与错误类型查询、WRONG_BOOK 专项会话出题全部通过；typecheck、lint、`next build` 全绿。
...
### 12.4 Batch 2 导入器落地说明

- `scripts/import-n5-vocabulary.ts`：BOM 感知 CSV 解析；筛 `level=N5`（718）；按本文 §4/§9 映射并幂等 upsert `KnowledgePoint`+`VocabularyEntry`+`VocabularyAcceptedForm`（主+次读音/表记）+主要 `VocabularySense`；`--validate` 干跑模式；输出完整报告。`scripts/validate-n5-vocabulary.ts` 为只校验入口，`scripts/check-n5-acceptance.ts` 对 `data/fixtures/vocabulary-acceptance.json` 断言。`package.json` 增 `db:validate:n5`/`db:import:n5`/`db:check:n5`。
- **清洗规则**：表记与读音字段按 `;`/`；` 拆分为多个 accepted form（主项=首个）；剥离括号注记（`(する)`、`(〜を)` 等 6 处读音）；去 BOM/首尾空白。`meaning_zh` 第一版整体存为单一主要义项，不按分隔符拆分（仅统计 multi-meaning 报告项）。`partOfSpeech`/`category`/默认例句留空，不据英文释义臆测。
- `sourceOrder` 取 `sourceKey` 末尾数字（如 `jlpt-n5-0049`→49），稳定且满足 `(level, sourceOrder)` 唯一。整行原始内容存入 `VocabularyEntry.rawDataJson`。
