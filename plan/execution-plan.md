# 执行计划

## 当前实施主线

单词已调整为独立学习模块，不再直接套用原有统一知识点的 `0-100` 单分数模型。

单词模块应优先完成免费基础背词闭环，再实现付费 AI 个性化例句。具体任务顺序、数据模型、判题规则、复习调度和 Ubuntu 验收清单见 `vocabulary-module-execution-plan.md`。

推荐当前顺序：

1. 单词数据模型和 N5 词汇导入。
2. 导入仓库统一词表中的 718 条 N5 来源词条，并完成单词本和详情查询。
3. 每日新词计划。
4. 客观题、分维度掌握和错误分类。
5. 到期复习和错词本。
6. AI 个性化例句。
7. AI 权益和额度。

以下原始 Phase 仍保留为项目整体路线参考；当其词汇部分与单词模块详细计划冲突时，以单词模块详细计划为准。

## Phase 0：准备环境

目标：Windows 本机只承担源码编辑，Ubuntu 虚拟机承担所有运行、验证和部署。

Windows 本机：
- 确认 Git 可用。
- 项目路径保持为 `D:\Project\Web\JPLearn`。
- 学习资料路径保持为 `D:\学习\日语`。
- 只查看、书写和修改源码与项目文档。
- 不安装项目依赖，不执行编译、构建、检查、测试、数据库操作、API 实际调用、服务启动或部署。

Ubuntu 虚拟机：
- 安装 Ubuntu Server 22.04 LTS 或 24.04 LTS。
- 启用 OpenSSH Server。
- 配置桥接网络或保证 Windows 可 SSH 访问。
- 安装 Node.js LTS 和 pnpm。
- 安装 Docker 和 Docker Compose。
- 承担项目依赖安装、编译、构建、检查、测试、数据库操作、API 调用、启动和部署。

验收：
- Windows 可以 `ssh 用户名@虚拟机IP`。
- Windows 项目目录不产生本机运行或构建产物。
- 虚拟机可以运行 `node --version` 和 `pnpm --version`。
- 虚拟机可以运行 `docker --version` 和 `docker compose version`。

## Phase 1：初始化 Web 项目

目标：建立 Next.js + TypeScript + Tailwind + Prisma 基础项目。

任务：
- 初始化 Next.js App Router 项目。
- 安装 Prisma、SQLite、OpenAI SDK、必要 UI 依赖；AI 配置按 DeepSeek v4 flash 的 OpenAI 兼容接口预留。
- 建立基础页面：首页、练习页、知识点列表页、设置页。
- 建立 `.env.example`。
- 建立基础 Dockerfile 和 compose 文件。

验收：
- 以下验收全部在 Ubuntu 虚拟机执行。
- `pnpm dev` 可启动。
- 首页可访问。
- `pnpm build` 通过。

## Phase 2：数据模型与导入

目标：把现有 N5 词汇和语法资料导入 SQLite。

任务：
- 编写 Prisma schema。
- 创建迁移。
- 编写词汇统一 CSV 与语法 CSV 的独立导入流程。
- 导入统一词表中的 718 个 N5 来源词条和 84 个语法知识点。
- 语法初始化通用掌握状态；单词掌握状态在首次分配、查询或操作时按需创建。

验收：
- 数据库内的 N5 词汇来源词条为 718 条，语法知识点为 84 条。
- 词汇和语法数量分别正确。
- 列表页能显示导入数据。
- 随机抽查 5 个词汇和 5 个语法字段映射正确。

## Phase 3：知识点浏览

目标：完成无 AI 依赖的核心浏览能力。

任务：
- 实现知识点列表接口。
- 实现知识点详情接口。
- 实现筛选和搜索。
- 展示掌握状态。
- 支持手动重置掌握状态。

验收：
- 可以按词汇/语法筛选。
- 可以按已掌握/未掌握筛选。
- 可以打开详情页查看完整信息。
- 无 AI 配置时页面仍可用。

## Phase 4：AI 例句生成

目标：为知识点生成并缓存例句。

任务：
- 建立 AI 客户端封装。
- 实现例句生成 prompt。
- 实现结构化 JSON 解析和错误处理。
- 实现例句缓存。
- 在详情页和练习页展示例句。

验收：
- 对任意知识点可生成 5 条例句。
- 第二次打开同一知识点优先使用缓存。
- AI 失败时显示错误并允许重试。

## Phase 5：练习批改与掌握度

目标：实现完整学习闭环。

任务：
- 实现练习抽题逻辑：未掌握优先，兼顾最近错误。
- 实现知识点到题型的映射逻辑，优先覆盖文本题。
- 实现用户理解题和造句题提交。
- 实现 AI 批改 prompt。
- 保存练习提交记录和逐知识点评估明细。
- 按评估明细逐项更新掌握状态。
- 对应知识点理解或使用正确加 20 分、错误减 10 分，达到 100 分后标记掌握。

验收：
- 用户可以完成一次练习。
- 正确/错误反馈和每个知识点的加减分明细能保存并展示。
- 掌握分数达到 100 后知识点变为已掌握。
- AI 调用失败不会更新掌握状态。

## Phase 6：虚拟机部署

目标：验证 Linux 部署路径，并通过 ngrok 提供测试访问。

任务：
- 把项目同步到 Ubuntu 虚拟机。
- 配置 `.env`。
- 当前开发阶段可直接使用 Node.js/pnpm 启动应用。
- 使用 ngrok 将虚拟机的 `3000` 端口映射为临时 HTTPS 公网地址。
- 在 Windows 和手机浏览器访问 ngrok 地址。
- 数据库功能完成后再加入 Docker Compose 和 SQLite 数据目录挂载。

验收：
- 虚拟机上应用可启动。
- Windows 和非局域网手机可通过 ngrok HTTPS 地址访问页面。
- 不依赖公网 IPv6、路由器端口转发或入站防火墙放行。
- 数据库功能完成后，数据库文件在容器重建后不丢失。
- AI 配置可用时能生成例句和批改。

## Phase 7：收尾与下一轮规划

目标：把 MVP 做到可长期自用。

任务：
- 补充 README。
- 补充开发和部署命令。
- 整理常见问题。
- 记录下一阶段功能候选。

下一阶段候选：
- 多用户登录。
- PostgreSQL。
- 间隔重复算法。
- 卡片/选择题模式。
- 导入更多等级资料。
- 移动端体验优化。
